...
11               SEGV        segmentation violation 에 발생되는 시그널. 유닉스를 프로세스가 사용하는 메모리 영역을 보호하는데, 다른 프로세스가 사용하는 메모리 영역을 침범하려고 할때 주로 발생
15               TERM       이 시그널을 받은 프로세스는 종료됨. 기본적인 동작은 KILL 과 같지만 KILL 과는 다르게 무시할 수도 있고, 추적해서 처리할 수도 있다.
