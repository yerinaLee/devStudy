...
> 네트워크 트래픽을 제어하기 위한 일종의 필터 역할을 하는 규칙 집합입니다. 네트워크에서 허용하거나 차단할 IP 주소, 포트, 프로토콜 등을 지정하여 보안을 강화하고 접근을 관리할 수 있습니다.
> 예를 들어, 특정 서브넷이나 IP 주소에서 특정 포트를 통해 들어오는 트래픽만 허용하거나 차단하는 등의 조건을 설정해, 네트워크 자원에 대한 접근을 제어하는 방식입니다. AWS 같은 클라우드 환경에서는 네트워크 ACL을 통해 **서브넷 단위**로 인바운드 및 아웃바운드 트래픽을 제어할 수 있습니다. 네트워크 ACL은 주로 보안 그룹과 함께 사용되며, 네트워크 ACL이 네트워크 레벨에서 기본적인 필터링을 제공하는 반면, 보안 그룹은 인스턴스 레벨에서 더 세밀한 제어를 제공하는 방식으로 차별화됩니다.


## - 서브넷 단위
> **서브넷 단위**는 IP 주소를 그룹화하여 네트워크를 구성하는 단위
> 일반적으로, 서브넷은 특정 IP 주소 범위를 나타내며, 이를 통해 네트워크 내에서 트래픽을 관리할 수 있습니다. 예를 들어, 하나의 서브넷은 192.168.1.0/24와 같이 표시될 수 있으며, 이는 192.168.1.0부터 192.168.1.255까지의 IP 주소를 포함하는 네트워크입니다.


## - 1. **latin1 (ISO-8859-1)**
> - **설명**: `latin1`은 서유럽 언어를 지원하는 인코딩으로, `ISO-8859-1` 또는 `ISO-Latin1`이라고도 불립니다.
> - **문자 범위**: 1바이트(8비트)를 사용하여 최대 256개의 문자를 표현할 수 있습니다.
> - **지원 언어**: 영어, 독일어, 프랑스어 등 서유럽 언어를 주로 지원합니다.
> - **특징**: 한글, 일본어, 중국어와 같은 다바이트(2바이트 이상) 문자가 필요한 언어는 표현할 수 없습니다. 주로 단순한 영어 텍스트와 서유럽 언어에서 사용됩니다.
> - **사용 사례**: 과거에 많이 사용되었으나, 지금은 유니코드(`utf8mb4`)가 널리 쓰이면서 점차 사용 빈도가 줄어들고 있습니다.

## - 2. **BIG5**
> - **설명**: `BIG5`는 주로 대만과 홍콩에서 사용되는 중국어(번체) 문자셋입니다.
> - **문자 범위**: 다바이트 인코딩을 사용하여, 한 문자를 1~2바이트로 표현합니다.
> - **지원 언어**: 중국어(번체자)를 표현하는 데 최적화되어 있습니다.
> - **특징**: 중국어 번체자 외에도 일부 문자는 추가로 지원하지만, 한글이나 일본어와 같은 다른 언어는 지원하지 않습니다. BIG5는 중국어 번체 문자를 위해 만들어진 문자셋이므로 한국어, 일본어와 같은 다른 언어에는 적합하지 않습니다.
> - **사용 사례**: 대만, 홍콩 등 번체 중국어 사용 지역에서 주로 사용되며, 현재는 `utf8` 또는 `utf8mb4` 같은 유니코드가 대체하고 있지만, 기존 시스템이나 특정 애플리케이션에서는 여전히 사용됩니다.


## - 서버 패치
> 서버 패치는 서버 쪽에서 이루어지는 업데이트입니다. 이 패치는 보통 서버 운영 시스템, 보안, 백엔드 기능 개선, 데이터베이스 구조 수정 등에 관련이 있어, 서버를 유지보수하고 관리하는 담당자가 서버에 직접 적용합니다. 게임이나 웹 서비스에서는 서버 패치 후 클라이언트가 아무 업데이트를 하지 않아도 바로 적용되는 경우가 많습니다.

## - 클라이언트 패치
> 클라이언트 패치는 사용자 쪽에서 업데이트가 필요한 경우입니다. 예를 들어, 사용자가 게임을 실행하거나 앱을 이용할 때 업데이트를 받아야 하는 경우가 이에 해당합니다. 클라이언트 패치는 보통 UI 변경, 기능 추가, 또는 그래픽 리소스 변경 등 사용자 인터페이스와 관련된 내용을 포함하고 있으며, 사용자가 직접 소프트웨어를 업데이트해야 변경 사항을 적용할 수 있습니다. 
> 
> 간단히 요약하자면, 서버 패치는 서버 쪽 업데이트라 사용자에게 별도의 다운로드가 필요 없고, 클라이언트 패치는 사용자 측 소프트웨어에 업데이트가 필요해 사용자가 직접 다운로드나 설치를 해야 한다는 차이가 있습니다.


## - DB 커넥션
> DB 커넥션(Database Connection)은 애플리케이션이 데이터베이스에 접근하기 위해 설정하는 연결 통로를 의미해요. 간단히 말하면, 프로그램과 데이터베이스 간의 **통신 채널** 같은 거죠. 
> DB 커넥션은 단순히 "연결" 이상의 의미가 있어요. 프로그램이 SQL 쿼리를 실행하거나 데이터를 조회할 때는 이 DB 커넥션을 통해 데이터베이스와 상호작용하게 됩니다. 하지만 이 커넥션은 무한정으로 제공될 수 있는 자원이 아니기 때문에, 커넥션을 장시간 점유하게 되면 다른 프로그램이나 사용자들이 접근할 수 있는 커넥션이 부족해질 수 있습니다.


## - 레지스트리(Registry)
> 레지스트리(Registry)는 **Windows 운영 체제**에서 시스템 설정과 애플리케이션 설정을 저장하는 **중앙 데이터베이스**입니다. 레지스트리는 **시스템, 하드웨어, 소프트웨어 설정**을 포함하는 중요한 정보를 저장하며, 이를 통해 운영 체제와 응용 프로그램들이 어떻게 동작하는지 정의합니다.
> 
> 레지스트리는 **키**(Key)와 **값**(Value)로 구성된 계층적인 구조를 가지고 있습니다. 이 구조는 **트리 구조**처럼 되어 있으며, 각 항목에는 이름, 데이터 타입, 실제 데이터 값이 저장됩니다.
> 
> ### 레지스트리의 주요 구성 요소
> 1. **키(Key)**: 레지스트리 내에서 정보를 저장하는 항목입니다. 각 키는 다른 키를 포함하거나 하위 키를 가질 수 있습니다.
> 2. **값(Value)**: 키에 연결된 실제 데이터입니다. 값은 다양한 데이터 유형을 가질 수 있습니다(예: 문자열, 정수, 이진 데이터 등).
> 
> 
> 레지스트리는 크게 다음과 같은 주요 **루트 키**들을 가지고 있습니다:
> - **HKEY_LOCAL_MACHINE (HKLM)**: 시스템의 하드웨어 및 소프트웨어 설정 정보.
> - **HKEY_CURRENT_USER (HKCU)**: 현재 사용자에 대한 설정 정보.
> - **HKEY_CLASSES_ROOT (HKCR)**: 파일 확장자, COM 객체 등과 관련된 정보.
> - **HKEY_USERS (HKU)**: 시스템의 모든 사용자 계정에 대한 설정 정보.
> - 



## - Endpoint
> Endpoint는 클라이언트가 요청을 보내는 **URL**을 지칭하며, 이를 통해 다양한 데이터 작업을 할 수 있게 됩니다.
> API가 두 시스템(어플리케이션)이 상호작용할 수 있게 하는 프로토콜의 총집합이라면, ENDPOINT는 API가 서버에서 리소스에 접근할 수 있도록 가능하게 하는 URL이라 할 수 있겠다. 
> - Endpoint는 인터페이스(사용자 조작 화면)에서 DB의 어떠한 자원에 CRDU동작을 하려고 할 때 필요한 구체적인 URL !



## - VPN
> VPN은 "Virtual Private Network"의 약자로, **가상 사설 네트워크**를 의미합니다. VPN은 공용 네트워크, 특히 인터넷을 통해서도 사설 네트워크처럼 안전하게 연결할 수 있도록 해줍니다.
> 
> ### VPN의 작동 원리
> VPN을 사용하면, 사용자의 인터넷 트래픽이 암호화된 터널을 통해 VPN 서버로 전달되고, VPN 서버를 통해 인터넷에 접속하게 됩니다. 이 과정을 통해 사용자의 실제 IP 주소가 숨겨지고 VPN 서버의 IP 주소로 대체되어, **익명성**과 **보안성**이 강화됩니다.
...
## - 아웃링크
> **해당 정보를 제공한 홈페이지로 이동하는 방식**을 아웃링크(outlink)라고함
...
## - 데드락 (Dead Lock)
> 데드락은 **두 트랜잭션 모두가 블로킹 상태에 진입하여 서로의 블로킹을 해결할 수 없는 상태**입니다. 트랜잭션 A, B에서 A가 B 트랜잭션에 대해 블로킹 상태로 진입한 경우에 B 트랜잭션이 종료(커밋 or 롤백)되어야 해당 블로킹이 끝나고 트랜잭션 A의 작업이 정상적으로 수행됩니다. 그러나 해당 상황의 B 트랜잭션에서 A에 대해 블로킹 상태로 진입한다면 마찬가지로 A 트랜잭션이 종료(커밋 or 롤백)되어야 해당 블로킹이 끝나고 트랜잭션 B의 작업이 정상적으로 수행됩니다. 이때 A와 B가 모두 상대 트랜잭션의 종료를 기다리고 있게 되어 서로의 블로킹을 영원히 해결할 수 없는 상태가 됩니다. 이러한 상황을 데드락 (Dead Lock)이라고 합니다.


## - LAD 파일
> 파일 확장자 LAD는일개의 주요 파일 형식이 있으며 (Daylon Graphics의) **Daylon Leveller**를 통해 열 수 있습니다. 이 형식과 연결된 총 일개의 소프트웨어 프로그램이 있습니다. 대부분 경우 이들은 **Daylon Leveller Animation Data** 형식입니다. 대부분의 LAD 파일은 **Graphic Files**로 고려됩니다.
> LAD파일을 Windows 플랫폼에서 찾았습니다. 이들 플랫폼은 데스크톱 PC (및 모바일) 장치 친화적입니다. LAD파일은 인기도가 "낮음"입니다. 이는 대부분의 장치에서는 자주 볼 수 없음을 의미합니다.
> LAD 파일에 대해 알아야 할 것이 많지만 문제 해결 방법이 이러한 형식의 파일을 다룰 시 가장 중요한 세부 사항입니다. 이러한 파일을 여는 데 문제가 있거나 더 자세한 정보가 필요하면 아래의 포괄적인 정보를 참조하십시오.


## - RDP 포트
> RDP 포트는 원격 데스크톱 프로토콜을 사용하여 원격 클라이언트와 서버 간의 통신을 용이하게 하는 네트워크 포트입니다. 기본적으로 RDP는 TCP 포트 3389를 사용합니다.


## - BTS (Bug Tracking System)
> 소프트웨어 개발 및 유지보수 과정에서 발생하는 버그, 이슈, 작업 항목 등을 관리하는 시스템을 의미합니다.  예: **JIRA**, **Redmine**, **Bugzilla** 등이 이에 해당합니다.


## - 피봇팅
> **기업이 처한 상황, 시장의 변화, 혹은 기술의 발전에 따라 사업 전략이나 모델을 근본적으로 변경하는 것을 말합니다**. 스타트업이 초기의 사업 모델이 예상대로 성과를 내지 못할 때, 새로운 기회를 탐색하고 적응하기 위해 방향을 전환하는 과정입니다.

## - Network Control Protocol (NCP)
> - **NCP (Network Control Protocol)**는 **ARPANET**과 같은 초기 네트워크에서 사용되었던 프로토콜로, 네트워크 상에서 연결을 설정하거나 제어하는 데 사용되었습니다.
> - **NCP**는 현대의 **TCP/IP** 프로토콜 스택에서 사용하는 **IP**나 **TCP** 같은 프로토콜이 도입되기 전에 **네트워크**와 **애플리케이션** 간의 통신을 관리하기 위한 방식이었습니다.
...
> 기존 운영 체제나 소프트웨어를 삭제하지 않고, 현재 설치된 상태를 유지하면서 더 높은 버전으로 업그레이드하는 과정을 말합니다. 주로 데이터와 설정을 그대로 유지할 수 있다는 점에서 새로 설치하는 방식인 클린 설치(Clean Install)와 구별됩니다.


## DMS (Database Migration Service)
> **시간, 리소스 및 비용을 절약할 수 있게 하나의 중앙 서비스에서 대규모로 데이터베이스와 데이터 웨어하우스를 계획, 평가, 변환 및 마이그레이션하는 기능** 


## DB 엔진
> DB에서 **엔진**은 데이터를 저장, 관리, 검색, 수정하는 데 사용되는 핵심 소프트웨어 구성 요소입니다. 데이터베이스 시스템의 "작동 방식"을 정의하며, 데이터 구조, 인덱싱, 트랜잭션 처리 등 다양한 기능을 제공합니다. MySQL에서 엔진은 특정 테이블이 데이터를 처리하는 방식을 결정합니다.
...
## 내부 DNS
> "내부 DNS"는 외부 인터넷에서 접근할 수 없는 사내 네트워크 내에서만 사용되는 도메인 이름 시스템(DNS)을 의미
> `api.abc.co.jp`와 같은 도메인이 외부에서는 접근할 수 없고, 내부 네트워크에서만 사용되는 도메인 이름이라는 뜻


## 비관적 lock, 낙관적 lock
>  낙관적 락 optimistic lock
>  : version 등의 구분 컬럼을 이용해 충돌을 예방함. 다른 트랜잭션에 의해 구분컬럼이 이미 업데이트되어있다면 해당 트랜잭션에서는 업데이트를 실행하지 않음
...
> 데이터 저장소로부터의 읽기와 업데이트 작업을 분리하는 패턴을 말한다. CQRS를 사용하면, 어플리케이션의 퍼포먼스, 확장성, 보안성을 극대화할 수 있다. 또한 CQRS 패턴을 통해 만들어진 시스템의 유연성을 바탕으로, 시간이 지나면서 지속적으로 시스템을 발전시켜나갈 수 있으며, 여러 요청으로부터 들어온 복수의 업데이트 명령들에 대한 충돌도 방지할 수 있다.
...
## 메타데이터
> 다른 자료에 대한 정보를 제공하는 데이터


## mocking
> 프론트에서, api 가 응답할 결과(JSON데이터)를 대신 만들어서 테스트하는것



## 동기/비동기, Blocking/Non-Blocking
> 동기/비동기 : 작업 순서의 처리 차이 (요청한 작업에 대해 완료 여부를 신경 써서 작업을 순차적으로 수행할지 아닌지)
> 블로킹/논블로킹 : 현재 작업이 block(차단, 대기) 되느냐 아니냐
...
## GPU, 그래픽카드 차이
> GPU는 그래픽 처리 장치의 주요 부품이고, 그래픽 카드는 그 GPU를 포함하여 그래픽 처리를 위한 모든 부품을 갖춘 완성된 장치


## 메타베이스 h2 DB
> 메타베이스 설치 시 별도 데이터베이스 설정이 없으면 기본적으로 사용되는 내장형 데이터베이스


## 데이터 웨어하우스, 데이터 레이크, 데이터 마트
> 서로 다른 클라우드 스토리지 솔루션
> 
> 데이터 웨어하우스 : 데이터를 정형 형식으로 저장. 사전 처리된 데이터 중앙 리포지토리
> 데이터 마트 : 특정 사업부의 요구 사항을 지원하는 데이터 웨어하우스입니다. 
> 데이터 레이크 : 원시 데이터 및 비정형 데이터의 중앙 리포지토리. 먼저 데이터를 저장하고 나중에 처리



## SPA(Single Page Application)
> 단일 페이지 모던 웹 애플리케이션. SPA는 웹사이트에 있는 이런 다양한 페이지를 하나의 페이지로 담는 것으로 사용자가 처음 웹사이트에 접속하면 핵심 정적 리소스(HTML, CSS, JavaScript)를 다운로드함. 이후 사용자가 다른 페이지로 이동하면, 페이지에서 변경이 필요한 부분만 로딩하고 전체 페이지를 다시 로딩하지 않음
...
## Polling 폴링 로직
> 폴링(polling)은 클라이언트가 주기적으로 서버에 요청을 보내 데이터를 확인하는 방식. 서버는 클라이언트 요청에 대해 새로운 데이터가 있는지 확인하고, 새로운 데이터가 있으면 이를 클라이언트에게 응답으로 보냄. 주로 실시간 데이터 처리가 중요한 경우 사용


## IP헤더
> 인터넷 프로토콜(IP) 패킷의 시작 부분에 있는 헤더 정보
> - 데이터를 패킷 단위로 잘라 전송할때, 패킷 : IP헤더 + 데이터 로 구성됨
> - IP헤더는 라우터/스위치 같은 네트워크 장치가 읽음
> - IP헤더는 패킷 내부의 layer 3로 낮은 레벨이라 cmd로는 완전히 보긴 어려움

## 홉
> 네트워크가 이동하는 경로. 단계 하나하나를 세는 단위
