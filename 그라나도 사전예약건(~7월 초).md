...
- 불러온 redis 정보 등과  preregisterInviteService.validateUserData() 호출
```
...
boolean checkPhoneExists = false;  
if("la".equals(postObj.getGname()) ||"astellia".equals(postObj.getGname()) || "cls".equals(postObj.getGname()) || "lostark".equals(postObj.getGname()) || "gj".equals(postObj.getGname()) || "xa".equals(postObj.getGname())){//astellia cls不須確認手機 + lostark 不須確認手機 // 전화번호를 통한 모바일 사전예약을 받는 경우에만 지나감. 그라나다도 모바일이 없으면 if 조건문에 추가해야될듯


boolean checkEmailExists = true; // 이메일로 사전가입 등록했는지 안했는지 체크. 이것도 게임명 추가해놔야할듯
