...
at index.jsp
1 - 로그인 여부에 따라 화면로딩 change

```
  <!---------------------未登入------------------------->   
<c:if test="${empty sessionScope.user}">  
  <div class="notSign">  
     <div class="sign1"><span>尚未登入</span> </div>  
      <a href="${pageContext.request.contextPath}/Index/Member/Login?ref=${pageContext.request.contextPath}/game/cls/preSignup/index" class="btn signbtn btn01" name="登入"></a>  
      <a href="${pageContext.request.contextPath}/Index/Member/Register?gname=${gname}"  target="_blank" class="btn signbtn btn02" name="註冊"></a>  
  </div>  
  </div>
  </c:if>  
  <!---------------------已登入------------------------->  
<c:if test="${not empty sessionScope.user}">  
  <div class="Sign">  
     <div class="sign1"><span>${sessionScope.user['UserID']} &nbsp;‧&nbsp;您好。</span></div>  
      <a href="${pageContext.request.contextPath}/Index/Member/Logout?ref=${pageContext.request.contextPath}/game/cls/preSignup/index"  class="btn signbtn btn03" name="登出 " ></a>  
  </div></div>  
</c:if>
```

아니근데 여기 contextPath가머야대체,,,
