...
1. 그라나도 사전가입
https://qa.mangot5.com/game/ge/preSignup/index
...
mapper연결 (src/com/jc/model/sqlmaps/preregisterInvite.xml) DB 데이터 저장
```
<insert id="Prelogin_insertMembersData" parameterClass="preregisterInvite">  
  INSERT INTO preregister_invite_withoutfb  
   (`area`,`phone_no`,`email`,`gname`,`event_code`,`device`,`username`)   VALUES   
   ('$area$','$phone_no$','$email$','$game_short_name$','$event_code$','$device$','$userName$')  
</insert>
```

저장후 service에서 result와 msg 저장 후 PreRegistInvitePostResultVO 객체 result 리턴
```
result.setResult("SUCCESS");  
if ("lostark".equals(postObj.getGname())) {  
    result.setMsg("恭喜您，您已成功完成事前預約。");  
}
```

-컨트롤러에서... 다시 돌아갈건데 코드가 지금안보임 ^_ 됐겠지머
...
2. ~~인텔리제이에 git 연결하는게 그럼 한 컴퓨터에서는 계정을 하나만 쓸 수 있는곤가,,,? -> nope
3. ~~jcgf 폴더에 /out 폴더는 뭐하는데지,,? ->빌드되면서 생기는 .class파일들
4. ~~todo 라고 주석 달아놓은건 뭐지...? -> 눈에 잘 띄라고 ㅋ
5. ~~fb는 뭐지...?(DB에 withoutFB) -> Facebook
...
-> https://qa.mangot5.com/report/presignup/index.jsp?game=ge
그라나다 사전가입 데이터추출 URL
...
Q
- [x] 모바일 페이지를 따로 만드는건가 -> nope!
- [x] 로그인버튼 어딧냐;;;
- [x] 유저타입에따른차이 두는지? -> 놉

- [x] 런칭 이벤트 따로 또 하는지? -> Redis Event2 URL 입력여부 -> 갑니다. 총 3개
- [ ] 개발기간은 어느정도로 두고보는지


### - coming soon alert 소스 위치

![[Pasted image 20240513183302.png]]

![[Pasted image 20240513183341.png]]

디자인팀에서만 사용하는 소스가 따로있음.
coming soon 뜨는건 여기있고, 이쪽 관련해서 코드에 문제있으면 디자인팀에 문의~


- **git 브랜치 내것으로 따오기**
PS C:\Users\admin\Desktop\projects\jcgf> git checkout -b branch/yeri/ISSUE_558

ge 연결 작업중 _ issue_558

OTP 문구수정 _  issue_566




checkAreaAndPhone = true 이거맞냐..?
